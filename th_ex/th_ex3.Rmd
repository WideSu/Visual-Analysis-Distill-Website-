...
packages = c('ggiraph', 'plotly', 'tidyverse', 'DT','gganimate',
             ...
             'd3scatter','tidycensus','timetk','ggseas','lubridate','ggrepel')
...
As we can see, people spend more money in **Feb-June**.

And most people **spend 30%-35%** of their **income per month**, and save the **70%**. And we can see that with the increase of income, people also spend more.
...
p7 <- ggplot(data=wage_info, aes(x = wage, y = Age_Group, fill = 0.5 - abs(0.5 - stat(ecdf)))) +
  ...
  ggtitle("Fig7. Distribution of wage with different Age Group")+
...
```{r}
personal_spending_monthly_cat <- financial %>% 
  filter(!category %in% c('Wage', 'RentAdjustment')) %>%
  group_by(month, category, participantId) %>%
  summarise(spend_cat = - sum(amount)) 
personal_monthly_income <- financial %>%
  filter(category %in% c('Wage', 'RentAdjustment')) %>%
  group_by(month,participantId) %>%
  summarise(income = sum(amount))
personal_monthly_spend <- financial %>%
  filter(!category %in% c('Wage', 'RentAdjustment')) %>%
  group_by(month,participantId) %>%
  summarise(spend = sum(abs(amount)))
personal_monthly_finance_status <- merge(personal_monthly_income,personal_monthly_spend,by=c("month"))
personal_monthly_finance_status$spendRatio <- personal_monthly_finance_status$spend / personal_monthly_finance_status$income
personal_monthly_finance_status$remain <- (personal_monthly_finance_status$income - personal_monthly_finance_status$spend)
personal_monthly_finance_status$remain <- round(personal_monthly_finance_status$remain, 1)
personal_monthly_finance_status <- personal_monthly_finance_status[order(as.Date(personal_monthly_finance_status$month, format="%m")),]
paged_table(personal_monthly_finance_status, options = list(rows.print = 15, cols.print = 5))
personal_cost_ratio <- merge(personal_spending_monthly_cat, personal_monthly_finance_status, by=c("month"))
personal_cost_ratio$spend_cat_ratio = personal_cost_ratio$spend_cat / personal_cost_ratio$income
paged_table(personal_cost_ratio, options = list(rows.print = 15, cols.print = 5))
```
